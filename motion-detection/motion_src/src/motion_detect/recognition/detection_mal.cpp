...
void * startRecognition(void * arg)
{
    ...
    directoryExistsOrCreate(cmd.c_str());
...
void * storenInstance(void * arg)
{
    ...
    cout << "day.instance_size():: " << day->instance_size() << endl;
    ...
    gettimeofday (&tr, NULL);
    ptmr = localtime (&tr.tv_sec);
    strftime (timeinfo, sizeof (timeinfo), "%Y-%m-%d %H:%M:%S %z", ptmr);
    ...
    for (int k = 0; k < day->instance_size(); k++)
    {
        ...
        std::string instancestart   = ins.instancestart();
        std::string instanceend     = ins.instanceend();
        ...
        sql_instance <<        
        "INSERT INTO instance (instancestart, instanceend, image_ids, time) " <<
